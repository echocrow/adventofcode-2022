...
function evalOptions(): void {
  ...
  const rate = item[2]!
  const opened = item[3]!
  if (rate > best[opened]!) best[opened] = rate
...
// Find best combo run.
let bestCombo = 0
for (let openedA = 0; openedA < best.length; openedA++) {
  for (let openedB = 0; openedB < best.length; openedB++) {
    if (!(openedA & openedB)) {
      const sum = best[openedA]! + best[openedB]!
      if (sum > bestCombo) bestCombo = sum
    }
  }
}
